...
// queryContext rebuilds, from the persisted task row, the context visible to
// query hooks. Polling has no relay info, so the task properties carry the
// model identities and the current data/state the driver hooks must see.
...
func (a *TaskAdaptor) ParseBatchResult(tasks []*model.Task, resp *http.Response, body []byte) (map[string]*service.BatchTaskResult, error) {
	...
	for _, item := range parsed {
		...
		if item.State != nil {
			...
			if marshalErr != nil {
				logger.LogWarn(context.Background(), fmt.Sprintf("task plugin %s rejected invalid poll state", a.plugin.Meta.Key))
			} else {
